...
import { useDynamicSettingsListLazy, useNotifier } from '../hooks';
...
export const DangerZone = ({
  ...
  const { refetch } = useDynamicSettingsListLazy({ environmentId: environmentId });
  ...
  const handleDelete = async () => {
    if (!id) return;
    try {
      await Dashboard.deleteDynamicSetting(id);
      await refetch();
      setShowDelete(false);
      router.push('/dynamic-settings');
    } catch (err) {
      if (err) {
      ...
      <CardContent>
        ...
        >
          <Dialog open={showDelete} onClose={() => setShowDelete(false)}>
            ...
            <DialogContent>
              <>
                API calls to this dynamic setting name will start failing. Are you sure you want to
                do this?
              </>
              <TextField
                fullWidth
