...
type Auth struct {
	...
	testAllowUnverifiedEmails     bool
}
...
type AuthOpts struct {
	...
	// For testing only since we can't verify generated accounts
	TestAllowUnverifiedEmails bool
}
...
func New(opts AuthOpts) (*Auth, error) {
	...
	return &Auth{client: client.Api(), allowedUnverifiedEmailMethods: allowedUnverifiedEmailMethods, allowedUnauthenticatedMethods: allowedUnauthenticatedMethods, testAllowUnverifiedEmails: opts.TestAllowUnverifiedEmails}, nil
}
...
func (a *Auth) authenticate(ctx context.Context, fullMethod string) (context.Context, error) {
	...
	session, res, err := a.client.V0alpha2Api.ToSession(ctx).XSessionToken(token).Execute()
	if err != nil {
	...
	if session.GetActive() {
		...
		if _, ok := a.allowedUnverifiedEmailMethods[fullMethod]; ok || a.testAllowUnverifiedEmails {
			return app_context.WithSession(ctx, session), nil
		}
	}
